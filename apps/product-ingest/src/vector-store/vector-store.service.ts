...
export class VectorStoreService implements OnModuleInit, OnModuleDestroy {
  ...
  async clearAllEmbeddings(): Promise<void> {
    ...
    try {
      this.#logger.debug('Executing DELETE query on product_embeddings table');
      const result = await pool.query('DELETE FROM product_embeddings');
      this.#logger.debug(`Deleted ${result.rowCount} existing embeddings`);
    } catch (error) {
      this.#logger.error(`Failed to clear embeddings: ${error instanceof Error ? error.message : 'Unknown error'}`);
      throw error;
    } finally {
      await pool.end();
    }
